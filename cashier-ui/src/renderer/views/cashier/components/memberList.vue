<template>
  <div class="member-container">
      ...
      <el-table-column label="操作" align="center" width="138" fixed='right'>
        <template slot-scope="scope">
          ...
          <el-button
            ...
            class="main-text"
            v-hasPermi="['cashier:index']"
            @click="openUpgradeMember(scope.row)"
          >升级</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 会员升级弹窗 -->
    <upgradeMemberDialog :show-dialog="showUpgradeDialog" :userId="upgradeUserId" @confirm="handleUpgradeConfirm" @closeDialog="showUpgradeDialog = false"/>
  </div>
</template>
...
import upgradeMemberDialog from "./upgradeMemberDialog";
import { Message } from "element-ui";
export default {
  name: "MemberIndex",
  components: { balanceRecharge, pointRecharge, upgradeMemberDialog },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    // 打开会员升级弹窗
    openUpgradeMember(row) {
      this.upgradeUserId = row.id;
    ...
    handleUpgradeConfirm(data) {
      ...
      this.$emit('upgrade-member', {
        gradeId: data.gradeId,
        payAmount: data.payAmount,
        gradeName: data.gradeName,
        userId: this.upgradeUserId
      });
    }
  }
};
